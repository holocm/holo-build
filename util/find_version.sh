...
if [ -d .git ]; then
    ...
    if [[ $root_basename =~ holo-build-* ]]; then
        echo ${root_basename/holo-build-/}
    else
        echo "Cannot determine application version. The root directory basename should look like 'holo-build-1.2.3', but actually is '$root_basename'." >&2
        exit 1
    fi
fi
