...
This switch has no effect when C<--suggest-filename> is in effect.
...
This switch has no effect when C<--stdout> or C<--suggest-filename> is in effect.
...
=item B<--rpm> (experimental)

Generate an LSB-compliant RPM package (suitable for Redhat, Fedora, Suse,
Mageia and derivatives). This is the default under suitable distributions.

B<WARNING:> The RPM generator is considered experimental because RPM is
...
=item B<--reproducible> (deprecated)

This switch does not have any effect anymore and will be removed in the next
major version. Packages are now always generated reproducibly.
...
dependency with C<except:>). For example, to have the package require all packages
...
For C<--pacman>, the same special syntax is allowed as for C<requires>; see
there for details.
...
B<BUG:> For C<--rpm>, specification of numeric IDs is broken. When encountering
a numeric user/group ID in a holo-build package, RPM will say things like
...
=item B<content> (string, required)
