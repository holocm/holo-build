...
format for the current distribution. Valid values are C<alpine>, C<debian>,
C<pacman> and C<rpm>. Note that C<alpine> does not support all features of the
package description format (see below).
...
targeted distribution:
    ...
    $ holo-build --suggest-filename --format alpine < input.toml
    foo-1.0-r1.apk
...
For C<--format=alpine>, the package name may contain C<[a-z0-9-]>, but the
...
=item B<epoch> (unsigned integer, not supported for C<--format=alpine>)
...
=item B<conflicts> (array of strings, not supported for C<--format=alpine>)
